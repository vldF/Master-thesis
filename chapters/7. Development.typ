...
#syntax-rule(
  "Синтаксические правила для ветвления",
  "syntax-if"
)[
```
<statement> ::= 
...
#table(
  ...
  "Detect", [
    _expression: any_ — выражение для выявления загрязнённых данных; #linebreak() _vulnerabilityType: string_ — тип уязвимости; #linebreak()
    _grammar: string_ — синтаксический признак данных.
  ],
  "Запускает обнаружение загрязнённых данных в выражении. В случае, если они будут обнаружены, анализатор вернёт уязвимость соответствующего типа. Грамматика определяет синтаксический тип данных, такой как URL, HTML текст, SQL.",
  "WithTaintMark<T>", [
...
На Рисунке @dataflow-diagram приведена верхоуровневая диаграмма потока данных, показывающая их преобразования в процессе трансляции.

#figure(
  caption: "Верхоуровневая диаграмма потока данных при трансляции",
  kind: "image",
  supplement: "Рисунок",
  image("../diagrams/dataflow-diagram.png")
) <dataflow-diagram>
