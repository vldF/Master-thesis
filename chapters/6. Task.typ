...
table(
  columns: 7*(auto, ),
  table.header(
    "Характеристика, подход", "YAML, JSON, XML", "Графически", "Существующий язык моделирования", "Язык общего назначения", "Существующий механизм расширений JSA", "Новый DSL"
  ),
...
Так как статический анализатор кода PT JSA применяет техники символьного исполнения, абстрактной интерпретации и анализа потока данных, необходимы языковые возможности для описания не только потоков загрязнённых данных, но и поведения описываемых компонентов.
...
class Request:
  def get_param(self, paramname: str) -> None:
    ...
    # имени; источник загрязнённых данных
    pass
