...
В Листинге @urllib3-descr объект _HTTPResponse_ описывает для PT JSA одноимённый класс библиотеки urllib3. Он содержит несколько полей, содержащие потенциально загрязнённые данные, такие, как _status_ и _data_. Листинг содержит объект типа _HTTPConnection_, а также несколько функций. К примеру, функция _json_ в urllib3 возвращает тело ответа на запрос, десериализованное из JSON в произвольный объект. С точки зрения taint-анализа, она является источником загрязнённых данных, по этому, в коде её результатом является результат вызова функции _CreateTaintedDataOfType\<any>_, которая создаёт такой объект произвольного типа. 
