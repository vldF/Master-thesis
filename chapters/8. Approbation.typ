...
Основной зоной применения PT JSA является анализ кода web-приложений на безопасность. Так как прототип DSL и его транслятора предназначены для описания библиотек и фреймворков на языке Python, проекты, приведённые в этой главе, также написаны на этом языке. Они показывают реальные способы написания кода бэкэнда. 
...
Для добавления поддержки библиотеки urllib3 в JSA был написан код на DSL, фрагмент которого приведён в Листинге @urllib3-descr. Он состоит из трёх файлов, которые подписаны комментариями. [TODO: ссылка на файл в репо]
...
Таблица @flask-res содержит уязвимости, обрануженные анализатором JSA на коде из Листинга @flask-app с разработанным расширением. 
