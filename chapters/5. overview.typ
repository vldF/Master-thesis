...
Несмотря на то что для этой технологии существует расширением для редактора кода VS Code, код правил, описанный на языке YAML, сложно читать и поддерживать. Также, модели позволяют описывать только потоки данных, но не имеют возможности для описания семантики библиотеки. У языка CodeQL отсутствует возможность быть интегрированным в другой анализатор.
...
В @flowdroid представлен инструмент для анализа безопасности приложений для ОС Android. Он реализует подход taint-анализа, для чего реализован механизм для описания источников, стоков и фильтрующих функций на языке XML. 
...
=== Существующие DSL <langs>

В @libsl описан язык LibSL, который позволяет описывать поведение внешних библиотек с использованием расширенных конечных автоматов. Язык предоставляет большой набор инструментов для расширения, включая типовые и семантические аннотации, семантические действия, библиотеки спецификаций. В @libsl_modeling авторы предлагают использование LibSL для аппроксимации поведения библиотек в различных задачах анализа программ, в том числе, в задачах taint-анализа. Несмотря на то, что данный язык хорошо зарекомендовал себя в аналогичной задаче, для интеграции с PT JSA его нужно было бы существенно изменить, ограничив существенную часть функциональных возможностей. Вместе с этим, необходимо расширить язык для того, чтобы описывать источники и стоки загрязнённых данных. Таким образом, интеграция LibSL в анализатор кода PT JSA является возможной, но повлечёт серьёзные изменения в языке.

В @piskachev2022fluently представлен DSL _fluent_\TQL, реализованный в виде набора классов и методов на языка Java. Он позволяет создавать спецификации библиотек для проведения taint-анализа. В @piskachev2021secucheck представлен подход к построению инструмента SecuCheck, использующего эти спецификации. Использование Java в качестве основного языка позволяет использовать уже существующие инструменты разработки, однако, приводит к возможности написания сложных для анализа спецификаций. Дополнительно к этому, в _fluent_\TQL использует особенный подъязык для описания сигнатур методов и классов, что требует наличия специализированных инструментов и затрудняет написание кода. Использование Java также делает затруднительным интеграцию этого DSL в PT JSA, который написан на C\#.

В @gotovchits2018saluki предложен DSL для анализатора кода Saluki. Он необходим для написания спецификаций функций с точки зрения источников и стоков загрязнённых данных. В языке реализована возможность указания шаблонов, с помощью которых можно задавать отношение между переменными в коде библиотек. Это делает язык формальным, что подходит для его формализации, однако усложняет его использование программистом. При этом, в нём отсутствует возможность описания поведения функций и классов, что необходимо для PT JSA.
